...
class ParticleFilter(object):
    ...
    def handle_observation(self, laser_scan, dt):
        ...
        for p in self.particles:
            ...
            errors.append(self.get_prediction_error_squared(laser_scan, p))
    ...
    def resample(self):
        ...
        for p in self.particles:
            ...
            while beta > self.weights[idx]:
                beta = beta - self.weights[idx]
                idx = (idx+1)%50

            p = deepcopy(self.particles[idx])
    ...
    def get_prediction_error_squared(self, laser_scan_msg, particle):
        ...
        subsample_ranges, subsample_angles = self.subsample_laser_scan(laser_scan_msg)

        # TODO: simulate a laser scan using one of the methods of this class
        predicted_ranges = self.simulate_laser_scan_for_particle(particle.x, particle.y, particle.theta, subsample_angles, self.laser_min_range, self.laser_max_range)

        # TODO: compute the difference bwteen predicted ranges and actual ranges
        # Take the squared norm of that difference
        error_vector = [a - b for a,b in zip(predicted_ranges,subsample_ranges)]
        norm_error = np.linalg.norm(np.array(error_vector))
        return norm_error**2
